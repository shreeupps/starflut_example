...
static int starCoreThreadCallDeep;
...
static NSString *fromString(VS_CHAR *val)
{
    if( val == NULL )
        return @"";
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  /*FlutterMethodChannel* */channel = [FlutterMethodChannel
      methodChannelWithName:@"starflut"
            binaryMessenger:[registrar messenger]];
  StarflutPlugin* instance = [[StarflutPlugin alloc] init];
    ...
    switch([index intValue]){
        ...
        case starcore_init :
        {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                ...
                StarCore_InitEx((VS_CHAR *)destDir,(VS_CHAR *)res_cpath);
                
#if defined(ENV_WITHRUBY)
                VSCoreLib_InitRuby();
#endif
#if defined(ENV_WITHPYTHON)
                //NSString *respaths = [[NSBundle mainBundle] resourcePath];
                //const VS_CHAR *res_cpath = [respaths UTF8String];
                VS_CHAR python_path[512];
                VS_CHAR python_home[512];
                sprintf(python_home,"%s/flutter_assets/starfiles/python",res_cpath);
                sprintf(python_path,"%s/flutter_assets/starfiles/%s.zip",res_cpath,ENV_WITHPYTHON);
                VSImportPythonCModuleDef CModuleDef[]={{"_hashlib",(void*)PyInit__hashlib},{"_ssl",(void*)PyInit__ssl},{NULL,NULL}};
                VSCoreLib_InitPython((VS_CHAR*)python_home,(VS_CHAR *)python_path,CModuleDef);
#endif
                VSCoreLib_Init( VS_TRUE, VS_TRUE, "",0, "", 0,NULL );
                SRPControlInterface = VSCoreLib_QueryControlInterface();
                
                SRPControlInterface->SetLocale("utf8");
                SRPControlInterface->SRPUnLock();
...
- (id)handleMethodCall_Do:(FlutterMethodCall*)call {
    ...
    switch([index intValue]){
        ...
        case starcore_InitCore:
        {
            ...
            VS_INT32 Result = VSCoreLib_Init( toBool([plist objectAtIndex:0]), toBool([plist objectAtIndex:3]),toString([plist objectAtIndex:4]),toInt32([plist objectAtIndex:5]),toString([plist objectAtIndex:6]),toInt32([plist objectAtIndex:7]), NULL);
            if( Result >= 0 ){
                ModuleInitFlag = true;
        ...
        case starcore_InitSimpleEx :
        {
            NSArray *plist = (NSArray *)call.arguments;
            SRPControlInterface -> SRPLock();
            VS_INT32 Result = VSCoreLib_Init(VS_TRUE, VS_TRUE, (VS_CHAR *)"", 0, (VS_CHAR *)"", toInt32([plist objectAtIndex:0]),NULL);
            if( Result < 0 ){
                SRPControlInterface -> SRPUnLock();
                return nil;
            }
        ...
        case starcore_InitSimple :
        {
            NSArray *plist = (NSArray *)call.arguments;
            SRPControlInterface -> SRPLock();
            VS_INT32 Result = VSCoreLib_Init(VS_TRUE, VS_TRUE, (VS_CHAR *)"", 0, (VS_CHAR *)"", toInt32([plist objectAtIndex:2]),NULL);
            if( Result < 0 ){
                SRPControlInterface -> SRPUnLock();
                return nil;
            }
        ...
        {
            if( ModuleInitFlag == false )
                return nil;
            [mutexObject lock];
            ExitAppFlag = true;
            [mutexObject unlock];
            [self removeFromObjectMap:NULL];
            if( BasicSRPInterface ->IsRootService() == VS_TRUE ){
        ...
        case starcore_regMsgCallBackP :
        {
            if( toBool(call.arguments) == false ){
                VSCoreLib_UnRegisterCallBackInfo(GlobalMsgCallBack,0);
            }else{
                VSCoreLib_RegisterCallBackInfo(GlobalMsgCallBack,0);
            }
            return nil;
        }
        ...
        {
            NSArray *plist = (NSArray *)call.arguments;
            VS_BOOL Result = SRPControlInterface ->SetRegisterCode(toString([plist objectAtIndex:0]),toBool([plist objectAtIndex:0]));
        ...
        case StarSrvGroupClass_setTelnetPort :
        {
            ...
            VS_BOOL RetResult = l_SrvGroup -> SetTelnetPort(toInt32([plist objectAtIndex:2]));
            return fromBool(RetResult);
        }
        ...
        case StarServiceClass_checkPassword :
        {
            ...
            l_Service ->CheckPassword( toBool([plist objectAtIndex:0]) );
            return nil;
        }
